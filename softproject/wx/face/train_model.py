...
def conv_net(x, weights, biases, dropout):
    ...
    # Convolution Layer
    conv1 = conv2d(x, weights['wc1'], biases['bc1'])      #128*128*24
    print(conv1.shape)
    # Max Pooling (down-sampling)
    conv1 = maxpool2d(conv1, k=2)               #64*64*24
    print(conv1.shape)
    # Convolution Layer
    conv2 = conv2d(conv1, weights['wc2'], biases['bc2'])        #64*64*96
    print(conv2.shape)
    # Max Pooling (down-sampling)
    conv2 = maxpool2d(conv2, k=2)               #32*32*96
    print(conv2.shape)
    ...
    fc1 = tf.nn.relu(fc1)
    # Apply Dropout
    fc1 = tf.nn.dropout(fc1, dropout) #deep_prob
...
# Construct model
pred = conv_net(x, weights, biases, keep_prob)  #softmax
...
with tf.Session() as sess:
    ...
    list = os.listdir("./resize_catimage/")
    print(list)
    print(len(list))
    count=0
    while count < 30:
        ...
        for batch_id in range(0, 2):
            ...
            for image in batch:
                ...
                #print(score)
                img = Image.open("./resize_catimage/" + image)
